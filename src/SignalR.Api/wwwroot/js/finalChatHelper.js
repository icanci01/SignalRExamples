function receiveAddNewRoomMessage(maxRoom, roomId, roomName, userId) {
    ...
    setTimeout(function () {
        ...
        if (changeTab) {
            let childli = ui.getElementsByTagName("li");
            let childDivs = divrooms.getElementsByTagName("div");
...
function readyPublicMessage(roomId) {
    ...
    sendButton.disabled = inputMsg.value.length === 0;
}
...
function receivePrivateMessage(
    ...
    let chatBoxName = `ulmessagesList${receiveId}`;
...
function receiveOpenPrivateChat(userId, userName, tabchange) {
    ...
    let newa = document.getElementById(`list-${userId}-list`);
    ...
    let div2 = document.getElementById(`list-${userId}`);
...
function notifyUnreadMessage(action) {
    ...
    if (pubtab.classList.contains("active")) {
        let badgeval = Number(badge.innerText);
        ...
        if (badgeval === 0) {
            ...
            badge.innerText = badgeval.toString();
        } else {
            badge.innerText = "99+";
        }
    }
